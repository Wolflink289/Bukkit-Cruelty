...
public class ParanoiaCommand extends BukkitCommand {
	
	@Override
	protected void handle(BukkitSender sender, String[] params) {
		if (!CrueltyPermissions.TRIP.canUse(sender)) {
		...
		if (!Cruelty.Attacks.PARANOIA.isEnabled()) {
		...
		*/
		...
		if (params.length != 1) {
			sender.getSender().sendMessage(CrueltyStrings.MSG_USG_PARANOIA); // Send without prefix
			return;
		}
		...
		sender.sendMessage(CrueltyStrings.MSG_ACT_PARANOIA.replace("${COUNT}", String.valueOf(suc)).replace("${PLAYERS}", sb.toString()));
	...
	static private boolean attack(Player player) {
		return Cruelty.attack(Cruelty.Attacks.PARANOIA, player);
	}
}