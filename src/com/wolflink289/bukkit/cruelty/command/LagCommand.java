...
public class LagCommand extends BukkitCommand {
	...
	protected void handle(BukkitSender sender, String[] params) {
		if (!CrueltyPermissions.LAG.canUse(sender)) {
			CrueltyPlugin.noPermission(sender);
			return;
		}
		
		if (!Cruelty.Attacks.LAG.isEnabled()) {
			sender.getSender().sendMessage(CrueltyStrings.MSG_ERR_DEPEND);
			return;
		}
		
		if (params.length != 1) {
			sender.getSender().sendMessage(CrueltyStrings.MSG_ACT_LAG); // Send without prefix
			return;
		}
		...
		sender.sendMessage(CrueltyStrings.MSG_ACT_LAG.replace("${COUNT}", String.valueOf(suc)).replace("${PLAYERS}", sb.toString()));
	...
	static private boolean attack(Player player) {
		return Cruelty.attack(Cruelty.Attacks.LAG, player);
	}
}
