...
public class CrashCommand extends BukkitCommand {
	...
	protected void handle(BukkitSender sender, String[] params) {
		...
		sender.sendMessage(CrueltyStrings.MSG_ACT_CRASH.replace("${COUNT}", String.valueOf(suc)).replace("${PLAYERS}", sb.toString()));
	...
	static private boolean attack(Player player) {
		return Cruelty.attack(Cruelty.Attacks.CRASH, player);
	}
}