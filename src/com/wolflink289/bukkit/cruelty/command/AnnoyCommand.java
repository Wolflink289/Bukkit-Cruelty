...
public class AnnoyCommand extends BukkitCommand {
	
	@Override
	protected void handle(BukkitSender sender, String[] params) {
		if (!CrueltyPermissions.ANNOY.canUse(sender)) {
			CrueltyPlugin.noPermission(sender);
			return;
		}
		
		if (!Cruelty.Attacks.ANNOY.isEnabled()) {
			sender.getSender().sendMessage(CrueltyStrings.MSG_ERR_DEPEND);
			return;
		}
		
		if (params.length != 1) {
			sender.getSender().sendMessage(CrueltyStrings.MSG_USG_ANNOY); // Send without prefix
			return;
		}
		...
		sender.sendMessage(CrueltyStrings.MSG_ACT_ANNOY.replace("${COUNT}", String.valueOf(suc)).replace("${PLAYERS}", sb.toString()));
	...
	static private boolean attack(Player player) {
		return Cruelty.attack(Cruelty.Attacks.ANNOY, player);
	}
}
