...
public class ScreamCommand extends BukkitCommand {
	
	@Override
	protected void handle(BukkitSender sender, String[] params) {
		if (!CrueltyPermissions.SCREAM.canUse(sender)) {
			CrueltyPlugin.noPermission(sender);
			return;
		}
		
		if (!Cruelty.Attacks.SCREAM.isEnabled()) {
		...
		sender.sendMessage(CrueltyStrings.MSG_ACT_SCREAM.replace("${COUNT}", String.valueOf(suc)).replace("${PLAYERS}", sb.toString()));
	...
	static private boolean attack(Player player) {
		return Cruelty.attack(Cruelty.Attacks.SCREAM, player);
	}
}
