...
public class DenyCommand extends BukkitCommand {
	...
	protected void handle(BukkitSender sender, String[] params) {
		if (!CrueltyPermissions.DENY.canUse(sender)) {
			CrueltyPlugin.noPermission(sender);
			return;
		}
		
		if (!Cruelty.Attacks.DENY.isEnabled()) {
			sender.getSender().sendMessage(CrueltyStrings.MSG_ERR_DEPEND);
			return;
		}
		
		if (params.length != 1) {
			sender.getSender().sendMessage(CrueltyStrings.MSG_USG_DENY); // Send without prefix
			return;
		}
		...
		sender.sendMessage(CrueltyStrings.MSG_ACT_DENY.replace("${COUNT}", String.valueOf(suc)).replace("${PLAYERS}", sb.toString()));
	...
	static private boolean attack(Player player) {
		return Cruelty.attack(Cruelty.Attacks.DENY, player);
	}
}
