...
public class TripCommand extends BukkitCommand {
	...
	protected void handle(BukkitSender sender, String[] params) {
		...
		sender.sendMessage(CrueltyStrings.MSG_ACT_TRIP.replace("${COUNT}", String.valueOf(suc)).replace("${PLAYERS}", sb.toString()));
	...
	static private boolean attack(Player player) {
		return Cruelty.attack(Cruelty.Attacks.TRIP, player);
	}
}