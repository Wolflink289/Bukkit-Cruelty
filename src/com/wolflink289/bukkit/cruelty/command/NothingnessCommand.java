...
public class NothingnessCommand extends BukkitCommand {
	
	@Override
	protected void handle(BukkitSender sender, String[] params) {
		if (!CrueltyPermissions.NOTHINGNESS.canUse(sender)) {
			CrueltyPlugin.noPermission(sender);
			return;
		}
		
		if (!Cruelty.Attacks.NOTHINGNESS.isEnabled()) {
			sender.getSender().sendMessage(CrueltyStrings.MSG_ERR_DEPEND);
			return;
		}
		
		if (params.length != 1) {
			sender.getSender().sendMessage(CrueltyStrings.MSG_USG_NOTHINGNESS); // Send without prefix
			return;
		}
		...
		sender.sendMessage(CrueltyStrings.MSG_ACT_NOTHINGNESS.replace("${COUNT}", String.valueOf(suc)).replace("${PLAYERS}", sb.toString()));
	...
	static private boolean attack(Player player) {
		return Cruelty.attack(Cruelty.Attacks.NOTHINGNESS, player);
	}
}
